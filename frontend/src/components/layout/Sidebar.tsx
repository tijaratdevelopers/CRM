...
export function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  function renderSignOut() {
    return (
      <div className={cn('border-t border-amber-900/25', collapsed ? 'p-2' : 'p-3')}>
        <button
          type="button"
      ...
      <nav className={cn('space-y-1', collapsed ? 'p-2' : 'p-3')}>
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              {isActive && !collapsed && (
                <span className="absolute -left-3 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-amber-300 shadow-[0_0_8px_rgba(245,196,69,0.9)]" />
              )}
              <item.icon
                className={cn(
  ...
  function renderFooter() {
    ...
    return (
      <div className="border-t border-amber-900/25 p-3">
        <div className="relative overflow-hidden rounded-xl border border-amber-700/25 bg-black/30 px-3 py-3 text-xs">
          <div className="flex items-center gap-2">
            <span className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-yellow-600 shadow-lg shadow-amber-500/25">
          ...
          <p className="mt-2 text-amber-100/60">Need help today? I&apos;m here to assist you.</p>
  ...
  return (
    <>
      ...
      >
        <div
          className={cn(
            'flex h-16 items-center gap-2.5 border-b border-amber-900/25',
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="relative z-50 flex h-full w-64 flex-col overflow-y-auto bg-sidebar-brand text-neutral-100 shadow-xl animate-slide-in-left">
            <div className="flex h-16 items-center gap-2.5 border-b border-amber-900/25 px-4">
              <BrandMark />
              <div className="flex min-w-0 flex-1 flex-col leading-tight">
                <span className="truncate text-sm font-extrabold tracking-tight text-white">
                  Tijarat <span className="text-primary">Developers</span>
